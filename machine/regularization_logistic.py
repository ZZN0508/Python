...
# @site :  
# @File : regularization_logistic.py
# @main : 逻辑回归正则化
# @Software: PyCharm
...
#画出最终分类的图
def plotBestFit(dataMat, labelMat,weights,weights1,weights2):
    ...
    r = weights1[0]
    a, b = (weights1[1], weights1[2])
    theta = arange(0, 2 * pi, 0.01)
    x = a + r * cos(theta)
    y = b + r * sin(theta)
    axes = fig.add_subplot(111)
    lin1,=axes.plot(x, y,color='red')

    r = weights2[0]
    a, b = (weights2[1], weights2[2])
    theta = arange(0, 2 * pi, 0.01)
    x = a + r * cos(theta)
    y = b + r * sin(theta)
    axes = fig.add_subplot(111)
    lin2,=axes.plot(x, y,color='yellow')
    axes.legend(handles=[lin0, lin1, lin2], labels=['原始图像', '正则化1', '正则化2'])
...
if __name__=='__main__':
    ...
    weights,weights1,weights2 = stocGradAscentss(dataMat, labelMat)
    plotBestFit(dataMat, labelMat,weights,weights1,weights2)